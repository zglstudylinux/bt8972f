...
4. **回环测试的适用边界**：本工程 PE7↔PB1 回环在 9.5M 的五种码型、5000 帧/2.56MB 加压中零误码，证明单块在途的物理层与时序能力；它不含"连续多块"与"外部时钟域"两个变量，不能替代双机测试。10M 回环已在非恒值码型误码，是独立的 PHY 边界。
...
| 层级 | 数字 | 依据 |
|---|---|---|
| 可编程上限 | **可配至 24M 且分频经 LA 脉宽实测准确**（12M=83ns、24M=41.7ns）；更高档未验证，已列原厂确认项。注意能配≠可靠：12M 起 TX 引擎 underrun、24M 内容未校验 | `uart2_huart_compare.md` §3.2 |
| 芯片能力（本工程 PE7↔PB1 回环，无适配器） | **9.5 Mbps 全 PASS**，10M 起误码 | 本工程 `huart_baud_test` 帧式回环：五码型 × 1000 帧 × 512B，错误/超时均 0（2026-09-14） |
...
### Q11：逻辑分析仪"采样率 8MS/s"是不是就能采 8M 波特率？

**不是，方向正好相反**：两个"M"是不同的东西——采样率是 LA 每秒打多少个采样点，波特率是线上每位多宽。解码要求**每一位至少覆盖 2~3 个采样点**（只落 1 个点时采样点稍偏就把 0 看成 1），可靠解码建议 4 个以上。换算表：

| 波特率 | 位宽 | 解码最低（2 样点/位） | 可靠（4 样点/位） |
| --- | --- | --- | --- |
| 2M | 500ns | 4MS/s | 8MS/s |
| 4M | 250ns | 8MS/s | 16MS/s |
| 6M | 166.7ns | 12MS/s | 24MS/s |
| 8M | 125ns | 16MS/s | 32MS/s（本装备无） |
| 12M | 83.3ns | 24MS/s（临界） | 48MS/s（无） |
| 24M | 41.7ns | 48MS/s（无） | — |

所以"采样率 8MS/s"的真实含义：**最多可靠解码 2M 波特率、极限尝试 4M；恰恰解不了 8M 波特率**（每位只摊到 1 个采样点）。要逐位解码 8M 波特率需 16MS/s 起步、24MS/s 才稳。

本场装备的三层限制（层层递进，2026-09-12 定稿，详见 serial_max_baud_test_plan.md 项 3）：
1. **采样率档位**：最高 24MS/s（官方标称）→ 理论可解码上限 8M（3 样点/位）~12M（2 样点/位，不可靠）；
2. **USB 持续流传输**：只撑到 ~16MS/s（24MS/s 全场 0/30+，重启/换口/换通道均无效）→ 实际可用上限 16MS/s；
3. **解码裕度**：还受流密度影响——背靠背满速率流比间隔流难解（HUART@6M/16MS/s 乱码 vs UART2@6M/16MS/s 间隔流零断裂），实战可靠解码上限 **4M 波特率**。

结论：8M+ 波特率的逐位线级验收需更换采集装备（Logic 8/Pro 级或更高）；目前 8M 以上以历史帧测（UART2 8M/12M/24M、HUART 8M×400 帧全对）与回环证据（9.5M）背书。
...
2. **接收缓冲结构**（这是连续流能力的本质差异）：UART2 RX 是**单字节缓冲、无 FIFO**——主循环必须在一个字节时间内取走，否则被下一字节覆盖，结构性无法接收连续流（实测无间隔突发捕获率 10%~57%，中断也救不回）；HUART 是**块缓冲（可调大小）+ 应用缓冲**，硬件自动连续收集——单块在途实测 100% 无损。吞吐差异同样由此而来：UART2 有效 ~28.6 KB/s（主循环节拍限制）vs HUART 线速至 8M TX（800 KB/s）。**同 9.5M 波特率满吞吐回环实测（2026-09-15）**：UART2 连续流 77 KB/s vs HUART 单块握手 375 KB/s / 背靠背 482 KB/s——同波特率下吞吐差 5~6 倍且双方零误码，量化了"高速"的实际意义（serial_max_baud_test_plan.md 项 2C）。
...
### Q12：板内回环测试为什么普通串口（12M）比高速串口（9.5M）上限还高？测试方法一致吗？

方法已完全统一（2026-09-14）：同为 **512B 帧 × 5 码型（00/FF/55/AA/递增）× 每码型
100 帧（筛选）/1000 帧（加压）**、1 stop、同一张 115200~12M 梯子（含 9M~12M 密集
边界档）、逐帧逐字节比对——不存在方法差异。结果：

| 外设 | 同口径最大无错档 | 边界 |
|---|---|---|
| UART2 | **12 Mbps**（12M 加压 2.56MB 零误码） | 24M 字节全到但内容误码 |
| HUART | **9.5 Mbps**（9.5M 加压 2.56MB 零误码） | 10M+ 非恒值码型全败、仅 0xFF 存活 |

为什么"高速"反而低——**先澄清一个容易讲错的归因**：两串口在线上都是异步通信
（线路不传时钟），自回环时 HUART 的 TX/RX 也是同一外设、同一分频配置、同一时钟源，
并不存在"回环路径上两个时钟域"的问题；手册 13.1 "Support async clock between UART
interface part and control part" 说的是 HUART **IP 内部**接口与控制部分可异步的架构
描述，不能直接作为 12M vs 9.5M 差异的因果依据（HUART 为原厂黑盒库，内部分频/采样
机制未公开，硅级确切机理已列原厂确认项）。

**站得住的汇报依据是失效形态 + 两个实测事实**：

1. HUART 在 10M+ 的失效形态极具辨识度：**只有恒值 0xFF 存活（线上几乎无跳变）、
   含交变边沿的码型（0x55/0xAA/递增）全部误码**——指向 10M 以上引擎/引脚的边沿与
   采样时序余量耗尽，而非随机干扰；且与同事独立报告（"10M 起误码、0xFF 最后失效"）
   及本工程历史记录（HUART 12M 起 TX 引擎 underrun）方向完全一致，边界真实；
2. UART2 的 12M 恰为 24MHz XOSC ÷2 **精确整数分频**，每位恰好 2 个时钟 tick、采样点
   确定；24M（÷1，每位 1 tick、无法中点采样）即误码——边界与"采样分辨率"理论精确
   吻合。UART2 结构简单（寄存器式、无内部异步桥），而 HUART 引擎复杂且为黑盒库，
   两者 pad 驱动/时序实现不同属正常。

"高速串口"指的是 **DMA 吞吐架构**（DMA 收发至 1024B、大流量数据流的真实吞吐远高于
UART2 的逐字节轮询），不是板内回环上限更高。注意口径：板内自回环收发同源，不含外部
时钟域变量，对外通信上限仍受对端时钟/适配器限制（见 Q10 三层口径）。

### Q13：UART2 自回环之前为什么全零失败？具体改了哪些寄存器？

初版 `bsp_uart2_com.c` 照抄原厂 UART1 **单线**模板，三处配置与双线全双工冲突。
经四轮寄存器级探针（手册 Register 12-1/12-2/12-3 逐位核对 + GPIO bitbang 正对照 +
pad 电平采样）定位，修复前后对比：

| 寄存器/配置 | 修复前（单线模板） | 修复后（双线全双工） | 为什么要改 |
|---|---|---|---|
| `UART2CON` bit6 ONELINE | 1（TX/RX one line） | **0（TX/RX separate）** | 单线=半双工：TX 波形能上线，但发送期间 RX 被硬件门控，自发自收不可能 |
| `GPIOEDIR` bit7（TX 脚方向） | 1（输入） | **0（输出）** | 双线模式下 pad 方向回归 GPIO DIR 寄存器：输入方向时 TXPND 在芯片内正常完成但 **pad 永不驱动**（手册 12.3 User Guide 第 1 步 "Set IO in the correct direction"）；单线模式外设自控 pad 方向，所以旧配置对外收发一直正常，唯独自回环全零 |
| `UART2CON[27:24]/[23:20]/[19:16]`（KEYIE/KEYEN/RSTEN） | `0xaaa`（三域写 0xa=key 检测功能全部使能） | `0x555`（三域写 0x5=全部关闭） | 手册 Register 12-1：这三个 4-bit 域是 key 匹配中断/key 模式/复位 key 模式开关，不是"KEY 魔法值"；排除干扰源 |
| `UART2BAUD` | `(baud<<16)\|baud` 高低 16 位都写 | 只写低 16 位 | 手册 Register 12-3：低 16 位才是分频（Baud=Fudet/(BAUD+1)），高 16 位 DARTBAUD 只读（硬件检波值），写之无意义 |
| `UART2CON` bit4 SB2EN | 1（2 stop bit） | 0（1 stop） | 与 HUART 对齐（统一口径）；也是出厂默认 |

定位过程要点（供以后排查借鉴）：GPIO bitbang 正对照证明短接线与 RX 事件路径有效；
pad 电平采样发现 **function 模式下 PE7 的 GPIO 读回不可信**（与短接网络上可信的 PB1
读数矛盾），判读应以可靠侧采样或 LA 为准；"TXPND 完成"只代表芯片内移位完成，不代表
pad 有波形——两者要分开验证。

修复后结果：统一口径筛选 115200~12M 五码型全 PASS、12M 加压 2.56MB 零误码；24M
字节全到但内容误码（走线眼图边界）。详见 serial_max_baud_test_plan.md 项 2B。

### Q14：波特率和"每一位时间"是什么关系？测试怎么适配不同波特率？

**波特率 = 线上每秒传输的位数，每一位的时间 = 1/波特率**：115200 → 每位 8.68µs；
1M → 1µs；12M → 83.3ns；24M → 41.7ns。一个 8N1 字节 = 1 起始位 + 8 数据位 + 1 停止位
= 10 位，故 115200 波特率下每秒最多 11520 字节（本板 UART2 用 2 stop 时为 11 位/字节）。

硬件产生位时间靠**分频器**，收发两端各自分频、必须落在容差内（约 ±2~3%）：

| 侧 | 位时间来源 | 115200 档示例 |
|---|---|---|
| 板端 UART2 | `波特率 = 24MHz XOSC ÷ (BAUD+1)`，BAUD=round(24M/波特率)-1 | N=208 → 实际 115384（+0.16%）；12M 档 N=2 精确无误差 |
| PC 适配器 | 脚本 `serial.Serial(port, baud)` 让 CH340/CP210x 按自身参考时钟分频 | 各自晶振决定精度 |

接收方用自己的时钟在每位中点采样（过采样），所以"换波特率"的本质就是改分频值——
固件里是梯子数组逐档调 `periph_init(baud)`，PC 侧是 `ser.baudrate = baud`。
另外注意：**回环测试的数据不经过任何脚本**——板子自发自发收，脚本只挂在 1.5M 固定
速率的 COM9 调试口抄文字结果；数据口适配器仅在"对外回传法"（项 1/项 2）中参与。
