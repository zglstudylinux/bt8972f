...
| **统一法实测（50KB 递增码流、双适配器交叉）** | **RX 最大无错 460800**（64B 突发+8ms 节流，单字节缓冲+轮询的结构极限）；**TX 实测 ≥1.5M 零误码**（2M+ 待 LA，历史帧测信号完整性达 24M） | `serial_max_baud_test_plan.md` §5 项 1（2026-09-11） |
...
| **统一法实测（50KB 递增码流、双适配器交叉）** | **RX 规格内最大无错 2M**（CH340 规格上限处板端 50760 全对，921600/1.5M 双适配器一致）；**TX 实测 ≥1.5M 双适配器零误码**（2M 档两颗适配器均接不住回传，待 LA） | `serial_max_baud_test_plan.md` §5 项 2（2026-09-11） |
