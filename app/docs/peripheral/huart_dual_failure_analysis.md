...
为什么"高速"反而低：两者是不同 IP。UART2 结构简单，直接 24MHz XOSC 整数分频
（12M=24M÷2 精确、8M=÷3），自回环收发同源同频、无时钟域差异，上限只取决于 pad 与
走线；HUART 接口与时钟域分离（手册 13.1 "async clock between UART interface part
and control part"），实测 10M+ 起逐位时序余量不足——失效形态（仅恒值 0xFF 存活、
非恒值全败）与同事独立报告完全同 signature，且历史有 12M TX 引擎 underrun 记录。
