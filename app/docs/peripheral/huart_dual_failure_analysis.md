...
汇报话术："普通串口能配到 24M 且波形正确，但那是物理层指标；受软件结构限制，**有效数据吞吐约 29KB/s，定位是低速命令/状态通道**。"
...
汇报话术："高速串口芯片能力**回环验证到 9.5M**；对外通信在现有适配器规格下 **1.5M 全码流零误码**，2M/3M 芯片侧已验证正确（损耗在适配器超规格）；发送单向验证到 **8M**。原厂手册未标官方上限，已列为确认项。"
