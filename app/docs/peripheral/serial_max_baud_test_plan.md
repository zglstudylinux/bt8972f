...
| UART2 TX（板→PC） | 24M（分频上限） | **≥1.5M**（双适配器回传零差异）；2M+ 待 LA | 历史 3M 逐位验证全对（16MS/s） | 信号完整性与吞吐分层：吞吐封顶 ~28.6KB/s 与波特率无关 |
| HUART RX（PC→板） | ≥9.5M（回环佐证） | **2M**（CH340 规格上限处板端全对；921600/1.5M 双适配器一致） | 未测（>2M 需 LA/更高速适配器） | 需 512B+10ms 节流（库非环形块缺陷）；回环证据 9.5M |
| HUART TX（板→PC） | ≥9.5M（回环佐证） | **≥1.5M**（双适配器回传零差异）；2M 双适配器均接不住回传，待 LA | 未测 | 历史帧测 8M PASS、12M underrun（huart_tx_bringup.md） |
