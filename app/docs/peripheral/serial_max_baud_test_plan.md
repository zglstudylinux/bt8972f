...
**回环结果（每档 32768B 递增码流自发自收，板端逐字节校验）**：

| 波特率 | got | err | drop | hw_pending | 判定 |
| --- | --- | --- | --- | --- | --- |
| 115200 / 230400 / 460800 / 921600 / 1M / 1.5M / 2M / 3M / 4M / 6M / 8M / **12M** | 32768 | 0 | 0 | 32768 | **全 PASS** |
| **24M** | 32768 | 32640 | 0 | 32768 | FAIL（字节全到、内容误码，wire 眼图闭合边界） |

**结论**：普通 UART2 板内短接回环最大无错波特率 = **12 Mbps**（分频 N=2）；24M 可配、
字节全收但内容误码。对照 HUART 板内回环 9.5M PASS / 10M 起误码（项 2A）——两个串口
板内 PHY 上限同为 10M+ 量级，远高于适配器链路的对外实测上限（受适配器规格限制，
见项 1/项 2 与 FAQ Q10 三层口径）。
