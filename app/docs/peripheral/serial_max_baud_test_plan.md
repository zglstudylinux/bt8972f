...
## 项 2B：UART2 同实例短接回环（PE7↔PB1，2026-09-14，已打通，最大无错 12M）

**根因（四轮寄存器级探针定位）**：初版驱动照抄原厂 UART1 单线模板，留下三处与双线
全双工冲突的配置——

1. `ONELINE=1`：单线半双工，TX 波形能上线（pad 采样到完整 0x55 帧、恰好 10 边沿），
   但发送期间 RX 被硬件门控——自发自收不可能；
2. TX 脚 GPIO 方向为输入：双线（ONELINE=0）模式下 pad 方向回归 GPIO DIR 寄存器，
   输入方向时 TXPND 在芯片内正常完成但 **pad 永不驱动**（手册 12.3 User Guide 第 1 步
   "Set IO in the correct direction" 即指此）；单线模式外设自控 pad 方向，故旧配置
   对外收发一直正常，唯独自回环全零；
3. `CON[27:16]` 写 `0xaaa` 把 KEYIE/KEYEN/RSTEN 三个 key 检测功能全部使能
   （手册定义为 0xa 使能/0x5 关闭），已改为 0x555 全关。

另按手册 Register 12-1/12-2/12-3 修正：BAUD 仅低 16 位为分频（高 16 位 DARTBAUD 只读）；
RX 有 4 字节缓冲（RX_BCNT[14:11]）与溢出标志 RX_4BUF_ERROR[15]，可作为驱动增强点。
探针各轮均以 GPIO bitbang 正对照（seen=1）证明短接/RX 路径有效，并发现 function 模式
下 PE7 的 GPIO 读回不可信（与短接网络上可信的 PB1 读数矛盾），pad 判读以可靠侧为准。

**驱动修复**（`bsp_uart2_com.c`，对单线/对外用法无副作用）：`ONELINE=0` 双线全双工、
PE7 输出方向、key 三域 0x555 全关、BAUD 只写低 16 位。探针验证 `data=0x55` 逐位精确命中。

**统一帧式口径（两外设完全一致，同日实测）**：512B 帧 × 5 码型（00/FF/55/AA/递增）×
每码型 100 帧（筛选）/ 1000 帧（加压），1 stop（SB2EN=0 与 HUART 对齐），逐帧逐字节
比对，连续 8 帧收不满提前终止；同一梯子 115200~12M 共 20 档（含 9M~12M 密集边界档）。
有效性对照：不短接时全档 got=0（测试不造 PASS）；短接复测多轮完全可重复。

**筛选结果（每码型 100 帧）**：

| 波特率 | UART2 | HUART |
| --- | --- | --- |
| 115200 ~ 9.5M（16 档） | 全 PASS | 全 PASS |
| 10M / 10.5M / 11M / 12M | **全 PASS** | FAIL（仅 FF 恒值通过——与同事报告"10M 起误码、0xFF 最后失效"同 signature） |
| 16M / 20M / 24M（仅 UART2 探底） | 16M 档全 PASS（实为分频量化后的 12M）；20M/24M 档实际均为 24M：非恒值码型误码、FF 存活 | — |

> UART2 分频只能产生 24M/N：16M 档量化为 12M、20M 档量化为 24M，故 16M"PASS"实为
> 12M 证据、20M"FAIL"实为 24M 证据，标注按名义值保留。

**加压结果（每码型 1000 帧 × 512B = 2,560,000B，五码型全 PASS 零误码）**：

| 外设 | 加压波特率 | 依据 |
| --- | --- | --- |
| **UART2** | **12 Mbps** | 本项统一帧式加压（2026-09-14） |
| **HUART** | **9.5 Mbps** | 项 2A 帧式加压（同口径参数） |

**结论**：同口径下板内回环最大无错波特率 **UART2 = 12 Mbps、HUART = 9.5 Mbps**——
方法统一后普通串口回环上限确实更高，不是测试假象。HUART 10M+ "仅 FF 恒值存活"的
失效形态与同事独立报告（10M 起误码、0xFF 最后失效）同 signature，边界真实；"高速"
命名指其 DMA 吞吐架构（独立时钟域、DMA 收发），不是回环上限更高。两串口板内 PHY
上限同为 10M+ 量级，远高于适配器链路的对外实测上限（受适配器规格限制，见项 1/项 2
与 FAQ Q10 三层口径）。UART2 驱动修复对单线/对外用法无副作用（ONELINE=0 下外部输入
接收已由探针正对照验证）；双板互测可作为独立时钟域的后续验证项。



## 项 2C：9.5M 满吞吐回环对比（2026-09-15）

同波特率（9.5M）下测两外设的**可达吞吐与误码**：连续自发自收、在线按位置比对
（0~255 递增图案，非恒值，丢字节/位错/错位均可检出），窗口 5s。理论线速 =
9.5Mbit/s ÷ 10 = **950 KB/s**（8N1）。

| 模式 | 5s 发送/实收 | 实测吞吐 | 误码 | 说明 |
| --- | --- | --- | --- | --- |
| UART2 连续流 | 386,740 / 386,613 | **77 KB/s** | 0 | CPU 逐字节搬运的结构上限（紧循环实测，含在线比对），线速仅用 8% |
| HUART SAFE（单块握手） | 1,876,480 / 1,876,480 | **375 KB/s** | 0 | DMA 块搬运 + 逐块握手，握手开销占大头 |
| HUART B2B（背靠背 DMA） | 2,466,816（TX） | **482 KB/s** | 0（校验窗内） | 受轮询逐块再发射延迟限制未到线速（非芯片上限） |

**结论**：
1. 同 9.5M 波特率下谁都不出误码（都在各自 PHY 上限内，见项 2B），差异体现在**可达
   吞吐**：HUART = 375~482 KB/s，是 UART2（77 KB/s）的 **5~6 倍**——这才是"高速
   串口"命名的实际意义（DMA 块搬运 vs CPU 逐字节轮询）；
2. B2B 段 512B 块 @9.5M + 轻量回调**未触发**重武装丢字节（细化 §2.3 缺陷表征：历史
   触发条件为 270B 小块 @3M + 较重回调；大块+轻回调可回避），环形模式（rxbuf_loop=1）
   为原厂连续流推荐方案，满线速 950KB/s 需 DMA 链式发射，后续可向原厂确认；
3. UART2 的 77KB/s 为紧循环上限（驱动结构），与 FAQ Q9 的 28.6KB/s（带业务主循环）
   同属"CPU 逐字节"机理，量级一致。
...
**UART2 分频精度（24MHz XOSC ÷ N，N=round(24M/baud)）**：1M~24M 全部为整除档，
**实际波特率零误差**（6M 档 N=4，位宽精确 166.7ns=1/6M）；921600 及以下档量化误差
+0.16%（如 921600→923077），远在 UART 容差内。LA 按标称值解码零断裂，即位宽真实的直接证据。
注意：位宽精度与**有效吞吐**是两回事——吞吐受驱动"每主循环 1 字节"节流
（回传法实测 ~28.6KB/s、TXLA 紧循环 ~165KB/s@6M），与波特率无关。
...
- **24MS/s 采样率在本装备上完全不可用**（全场 0/30+，历经 Logic 2 软件重启、更换 USB 口、
  CH1→CH0 换通道验证均无效；8/12/16MS/s 同环境可用）——判定为设备与 USB 控制器兼容性/
  带宽限制，官方标称 24MS/s 需标准 USB 2.0 环境支撑。**8M+ 档位的逐位线级验收需更换
  采集装备**，目前以历史帧测（UART2 8M/12M/24M、HUART 8M×400 帧）与回环证据（9.5M）兜底；
- 孤立事件噪声地板：部分采集出现 0~4 处/20~40 万字节的孤立断裂（BER ~10⁻⁵~10⁻⁶ 量级、
  位置随机），已贴近 LA 自身采样噪声，无法归因板端，报告中如实标注不计为误码证据；
