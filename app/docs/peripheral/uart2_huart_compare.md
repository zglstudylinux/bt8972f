...
| **RX 可靠输入** | 无 FIFO，主循环轮询 | **字节间隔 ≥1ms** 才可靠（≈1 KB/s 稳态）；无间隔突发捕获率仅 **10%~57%** | DMA 块接收 | **无间隔突发（≤块长）固件侧 100% 捕获**；连续多块流（>块长背靠背）库非环形模式从第 2 块起错位，需逐块流控（1.5M 实测全过）或 `rxbuf_loop=1`（见 huart_rx_bringup.md §4.1） |
