...
#define EQ_COEF_LEN(band)               ((5*band+1) * 4)
#define EQ_ZX_LEN(band)                 (2*(band+1) * 4)
#define DYEQ_COEF_LEN                   (4*(1+5) * 4)
#define DYEQ_ZX_LEN                     ((20+16) * 4)
#define DRC_COEF_LEN                    (20 * 4)
#define FIR_COEF_LEN(order)             (order * 4)
#define FIR_ZX_LEN(order)               (order * 4)
...
//常用DRC数据格式
#define DRC_ZX_BUF_SIZE         0
#define DRC_COEF_BUF_SIZE       DRC_COEF_LEN
typedef struct {
    drc_coef_t drc_coef;
} pacc_drc_bd_t;
...
//cs_list
bool pacc_cs_list_set_io_fmt(pacc_cs_t *start_cs, bool stereo, u8 in_fmt, u8 out_fmt);  //设置cs链路输入输出格式
