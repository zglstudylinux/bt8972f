...
#define REC_OBUF_SIZE                (0x800  + 0x800)             //ADC PCM缓存BUF SIZE
#define REC_ENC_SIZE                 (0x1A00 + 0x1000)            //录音压缩数据缓存BUF SIZE
#define REC_SYNC_TIMES               1                            //间隔5S录音时间同步一次

#define WAVE_FORMAT_PCM              0x0001
#define WAVE_FORMAT_DVI_ADPCM        0x0011
#define PCM_BITS                     (32)                        //录音文件格式为16bit
#define DATA_CKID                    0x61746164

#define REC_TYPE_SEL    REC_WAV
#define WRITE_2K    0

#define MIC_LFN_U_NUM       12      //112234
#define MIC_SFN_S_NUM       6
#define MIC_LFN_H_NUM       22      //*R20210412-//*表示Unicode开头长度变量
#define MIC_SFN_H_LEN       10
...
typedef struct {
    ...
    BYTE    cshift;         /* clust size多少个512的移位*/
    BYTE    sshift;         /* sector size多少个512的移位 */
    BYTE    genflags;       /* ExFAT General Secondary Flags */
    WORD    n_rootdir;      /* Number of root directory entries (0 on FAT32) */
    DWORD   csize;          /* Number of sectors per cluster */
    DWORD	fatsize;	    /* Size of an FAT [sectors] */
	...
	WORD    max_preclust;   /* 最大预分配簇 */
    WORD    remain_clust;   /* 预分配簇大小 */
	DWORD	dir_sect;		/* Sector number containing the directory entry  */
	BYTE*	dir_ptr;		/* Pointer to the directory entry in the win[]  */
	DWORD   fattime;        /* FAT DATE & TIME, 创建文件时使用*/
#endif
