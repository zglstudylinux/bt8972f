...
void bt_tws_sync_fot_data(void)
{
    ...
    while (fot_tws_data_size) {
        ...
        if (continue_flag) {
            ...
            wireless_send_data_until_success(0, fot_tws_ptr+fot_tws_ptr_pos, fot_tws_frame_len+1);
            wireless_send_data_until_success(1, fot_tws_ptr+fot_tws_ptr_pos, fot_tws_frame_len+1);
        } else {
            wireless_send_data_until_success(0, fot_tws_ptr+fot_tws_ptr_pos, fot_tws_frame_len);
            wireless_send_data_until_success(1, fot_tws_ptr+fot_tws_ptr_pos, fot_tws_frame_len);
        }
...
AT(.text.fota.update)
void fot_tws_sync_update_err(void)
{
    u8 data[2];

    data[0] = FOT_TWS_CMD_TAG;
    data[1] = FOT_TWS_UPDATE_ERR_SYNC;
...
void fot_tws_cmd_deal(u8 index, u8 *buf, u16 len)
{
    ...
    switch(cmd){
        ...
        case FOT_TWS_UPDATE_ERR_SYNC:
            fot_var.flag |= (FOT_FLAG_UPDATE_EXIT | FOT_FLAG_SYS_RESET);
            fot_var.tick = tick_get();
            break;
...
void fot_tws_recv_proc(u8 index, u8 *buf, u16 len)
{
    ...
    if(buf[FOT_CMD_POS] == FOT_OUT_DATA_START){
   		if(tx_ota_start_flag == 0) {
			tx_ota_start_flag = 1;
   		}
   		fot_var.tws_update_ticks = tick_get();
        FOT_DEBUG("get fot start\n", fot_var.remain_len);
...
void bsp_fot_process_do(void)
{
    ...
    if (tx_ota_start_flag) {
        if (tick_check_expire(fot_var.tws_update_ticks, 120000)) {
            FOT_DEBUG("--->fot timeout\n");
            fot_var.tws_update_ticks = tick_get();
            tx_ota_start_flag = 0;
            WDT_RST();
            uds_iap_reset();
...
void bsp_fot_flag_do(void)
{
    ...
    if(fot_var.flag & FOT_FLAG_CLK_SET){
        ...
        sys_clk_req(INDEX_FOTA, SYS_120M);        //先抬高主频
        fot_var.flag &= ~FOT_FLAG_CLK_SET;
    }
